...
msgbox "*just as you think you're safe, the squad comes rushing into your house.*"
msgBox "*they break in through the window, then try to flash a memory eraser in your face.*"
decision=inputBox("Do you resist (A) or don't (B) ?")
if decision="A" then
	msgbox "*you snatch the memory eraser from the operative's hand and erase the operative's memory to save your memories*"
	msgbox "*the operator faints and falls on your bed, and before he wakes up, you carry him outside.*"
	msgbox "*he wakes up but walks away like nothing happened.*"
end if
if decision="B" then
	msgbox "*the operative erases your memory*"
	msgbox "What? Where am I? What's my name?"
	Wscript.quit
end if
